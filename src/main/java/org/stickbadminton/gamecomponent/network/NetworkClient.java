...
//    private volatile boolean isP1 = false;
...
//            isP1 = clientId.equals("p1");
//            System.out.println("[Client] Assigned as " + clientId);
//        } else if (line.startsWith("BALL_UPDATE:")) {
//            String[] parts = line.split(":");
//            if (parts.length >= 5) {
//                ball.x = Double.parseDouble(parts[1]);
//                ball.y = Double.parseDouble(parts[2]);
//                ball.speedX = Double.parseDouble(parts[3]);
//                ball.speedY = Double.parseDouble(parts[4]);
//                isMyTurn = isPlayerTurn(clientId, ball.x, ball.y);
//                System.out.println("[Client] Ball at (" + ball.x + ", " + ball.y + ")");
//            }
//        } else if (line.startsWith("SCORE_UPDATE:")) {
//            String[] parts = line.split(":");
//            if (parts.length >= 3) {
//                p1Score = Integer.parseInt(parts[1]);
//                p2Score = Integer.parseInt(parts[2]);
//                System.out.println("[Client] Score: " + p1Score + "-" + p2Score);
//            }
...
//        if (connected) {
//            out.println("KEY_DOWN:" + key);
//        }
//    }
//
//    public void shoot(double x, double y, double angle, boolean heavy) {
//        if (connected && isMyTurn) {
//            String side = isP1 ? "p1" : "p2";
//            out.println("SHOOT:" + x + ":" + y + ":" + angle + ":" + heavy + ":" + side);
//        } else {
//            System.out.println("[Client] Not my turn to hit.");
//        }
//    }
//
//    public void requestScoreIncrement(int side) {
//        if (connected) {
//            out.println("SCORE_INCREMENT:" + side);
//        }
//    }
//
//    public void disconnect() {
...
//        client.shoot(100, 200, 45, true); // p1 击球
//
//        // 模拟得分
//        Thread.sleep(5000);
//        client.requestScoreIncrement(2); // p2 得分
//
//        // 模拟按键
//        Thread.sleep(1000);
//        client.sendKeyDown("LEFT");
//        Thread.sleep(1000);
//        client.sendKeyDown("RIGHT");
//
//        // 保持连接
//        Thread.sleep(10000);
//        client.disconnect();
 ...
 * - 捕获本地物理输入，仅向服务端上报
 * - 仅根据服务端的广播/回显来注入按键和派发连接/游戏事件
 * - 新增：支持比赛权威状态（SCORE/SERVE/BALL）回调与上报（由 p1 发送）
 *
 * 为了兼容仓库里的旧调用，保留 start()/sendReady()/sendSelect()/sendStartRequest()/attachToPrimaryStageAuto() 的别名或空实现。
 */
public class NetworkClient {
    ...
    public interface GameplaySyncListener {
        ...
        void onBall(double x, double y, double vx, double vy);
    }
    ...
    public void attachScene(Scene scene) {
        ...
        scene.addEventFilter(KeyEvent.KEY_PRESSED, e -> {
            ...
            // 客户端白名单软校验：避免无效/越权按键淹没网络（服务器仍会硬校验）
            if (!isKeyAllowedForSelf(code)) {
                return;
            }
            ...
            // 物理输入不直接作用于本地游戏，由服务器回显为准，故消耗事件防止本地与权威状态分叉
            e.consume();
        });
    ...
    // —— 新增：权威端（p1）上报比赛状态 —— //
    public void sendScoreUpdate(int left, int right) {
        sendLine("SCORE:" + left + ":" + right);
    }

    public void sendServeSide(int side) {
        // 规范化到 1/-1
        sendLine("SERVE:" + (side >= 0 ? 1 : -1));
    }

    public void sendBallState(double x, double y, double vx, double vy) {
        sendLine(String.format(Locale.US, "BALL:%.3f:%.3f:%.3f:%.3f", x, y, vx, vy));
    }
    ...
    // 旧调用：netClient.sendStartRequest() —— 现由服务器自动判定，保持空实现
    public void sendStartRequest() { /* no-op: server auto START when conditions met */ }
            ...
            if (line.startsWith("SCORE:")) {
                ...
                if (parts.length == 3) {
                    try {
                        ...
                        if (gl != null) {
                            runOnFxThreadOrNow(() -> gl.onScore(l, r));
                        }
                    } catch (NumberFormatException ignored) {}
                }
                return;
            ...
            if (line.startsWith("SERVE:")) {
                ...
                if (parts.length == 2) {
                    try {
                        ...
                        GameplaySyncListener gl = gameplayListener;
                        if (gl != null) {
                            runOnFxThreadOrNow(() -> gl.onServeSide(side >= 0 ? 1 : -1));
                        }
                    } catch (NumberFormatException ignored) {}
                }
                return;
